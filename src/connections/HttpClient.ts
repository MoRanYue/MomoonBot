...
export class HttpClient extends Client {
  ...
  public send(action: ConnectionEnum.Action.createGroupFileFolder, data?: null | undefined, cb?: DataType.ResponseFunction<ConnectionContent.ActionResponse.CreateGroupFileFolder> | undefined): void;
  ...
  public send(action: string, data?: string | Record<string, any> | null | undefined, cb?: DataType.ResponseFunction<any> | DataType.RawResponseFunction<any> | undefined): void {
    if (!this.host) {
      throw new ActionFailedError(action, undefined, undefined, `客户端没有指定请求地址`)
    }
    ...
    this.inst.post(this.host + action, data).then(res => {
      if (res.status != 200) {
        throw new ActionFailedError(action, res.status, true)
      }
      ...
      try {
        ...
        throw new ActionFailedError(action, undefined, undefined, "返回数据的结构不正确")
      }
      ...
      if (result.status == ConnectionEnum.ResponseStatus.failed) {
        throw new ActionFailedError(action, result.retcode, undefined, result.data)
      }
